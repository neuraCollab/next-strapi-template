...
export async function BlogLayout({ article, locale, children }: { article: Article; locale: string; children: React.ReactNode }) {
  return (
          ...
          <article className="pb-8 pt-8">
            <div className="flex gap-4 flex-wrap ">
              {article.categories?.map((category, idx) => (
                <p key={`category-${idx}`} className="text-xs font-bold text-muted px-2 py-1 rounded-full bg-neutral-800 capitalize">
                  {category.name}
                </p>
              ))}
            </div>
            <header className="flex flex-col">
              <h1 className="mt-8 text-4xl font-bold tracking-tight text-neutral-200 sm:text-5xl ">{article.title}</h1>
            </header>
            ...
            <div className="flex space-x-2 items-center pt-12 border-t border-neutral-800 mt-12">
              ...
              <time dateTime={article.publishedAt} className="flex items-center text-base ">
                <span className="text-muted text-sm">{format(new Date(article.publishedAt), "MMMM dd, yyyy")}</span>
              </time>
            </div>
          </article>
        </div>
