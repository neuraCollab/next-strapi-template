    ...
    <div className="w-full py-20">
      ...
      <div className="divide-y divide-neutral-800">
        {results.length === 0 ? (
          ...
          results.map((article, index) => <BlogPostRow article={article} key={article.slug + index} />)
        )}
      </div>
    </div>
  )
...
export const BlogPostRow = ({ article }: { article: Article }) => {
  return (
    ...
    >
      <div>
        <p className="text-neutral-300 text-lg font-medium group-hover:text-white transition duration-200">{article.title}</p>
