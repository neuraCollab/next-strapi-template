    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          <p className={cn("font-medium", plan.featured && "text-black")}>{plan.name}</p>
          {plan.featured && (
            <div className={cn("font-medium text-xs px-3 py-1 rounded-full relative bg-neutral-900")}>
        ...
        <div className="mt-8">
          {plan.price && <span className={cn("text-lg font-bold text-neutral-500", plan.featured && "text-neutral-700")}>$</span>}
          <span className={cn("text-4xl font-bold", plan.featured && "text-black")}>{plan.price || plan?.CTA?.text}</span>
          {plan.price && (
            <span className={cn("text-lg font-normal text-neutral-500 ml-2", plan.featured && "text-neutral-700")}>/ launch</span>
          )}
        </div>
        <Button
          variant="outline"
      ...
      {plan.additional_perks && plan.additional_perks.length > 0 && <Divider featured={plan.featured} />}
      <div className="p-4">
...
const Step = ({ children, additional, featured }: { children: React.ReactNode; additional?: boolean; featured?: boolean }) => {
  return (
    <div className="flex items-start justify-start gap-2 my-4">
      ...
      <div className={cn("font-medium text-white text-sm", featured && "text-black")}>{children}</div>
    </div>
  )
}
...
const Divider = ({ featured }: { featured?: boolean }) => {
  return (
    <div className="relative">
      <div className={cn("w-full h-px bg-neutral-950", featured && "bg-white")} />
      <div className={cn("w-full h-px bg-neutral-800", featured && "bg-neutral-200")} />
      <div
        className={cn(
      ...
      >
        <IconPlus className={cn("h-3 w-3 [stroke-width:4px] text-neutral-300", featured && "text-black")} />
      </div>
    </div>
  )
}
