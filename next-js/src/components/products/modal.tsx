...
export default function AddToCartModal({ onClick }: { onClick: () => void }) {
  ...
  return (
    <Modal>
      ...
      <ModalBody>
        <ModalContent>
          <h4 className="text-lg md:text-2xl text-neutral-600 font-bold text-center mb-8">Your cart</h4>

          {!items.length && <p className="text-center text-neutral-700">Your cart is empty. Please purchase something.</p>}
            ...
            {items.map((item, index) => (
              <div key={"purchased-item" + index} className="flex gap-2 justify-between items-center py-4">
                ...
                <div className="flex items-center">
                  ...
                  />
                  <div className="text-black text-sm font-medium w-20">${formatNumber(item.product.price)}</div>
        ...
        <ModalFooter className="gap-4 items-center">
          <div className="text-neutral-700 ">
            total amount <span className="font-bold">${formatNumber(getCartTotal())}</span>
          </div>
          <button
